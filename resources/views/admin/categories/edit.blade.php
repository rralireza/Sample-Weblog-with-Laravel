...
@section('container')
    <div class="jumbotron shade pt-5">
        ...
        <hr class="mt-0 mb-4">
        <form class="p-2" action="{{ route('categories.update' , $category) }}" method="post">
            @csrf
            @method('PATCH')
            <div class="form-group bmd-form-group">
                <label for="exampleFormControlInput1" class="bmd-label-static">عنوان</label>
                <input type="text" name="title" class="form-control" id="exampleFormControlInput1" placeholder="عنوان دسته بندی جدید را وارد کنید" value="{{ $category->title }}">
            </div>

            <div class="form-group bmd-form-group">
                <label for="exampleFormControlInput1" class="bmd-label-static">نامک</label>
                <input type="text" name="slug" class="form-control" id="exampleFormControlInput1" placeholder="نامک (اسلاگ) دسته بندی جدید را وارد کنید" value="{{ $category->slug }}">
            </div>
